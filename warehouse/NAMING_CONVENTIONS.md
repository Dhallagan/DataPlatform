...
- `silver`: staging views (`stg_*`) + core entities/facts/dims
...
- `dim_`: dimensions in `silver`
- `fct_`: event/transaction facts in `silver`
- `bridge_`: cross-entity relationship tables in `silver`
...
- Finance: `fin.mrr`, `fin.monthly_revenue`
...
- Keep canonical facts/dims in `silver`; build domain models in gold schemas.
