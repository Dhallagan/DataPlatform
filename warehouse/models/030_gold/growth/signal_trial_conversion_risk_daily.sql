...
org_usage_7d AS (
    SELECT
        ...
        SUM(CASE WHEN s.is_successful THEN 1 ELSE 0 END) AS successful_runs_7d,
        SUM(COALESCE(s.error_count, 0)) AS error_events_7d,
        MAX(s.session_date) AS last_run_date
    FROM {{ ref('sessions') }} s
    WHERE s.session_date >= CURRENT_DATE - INTERVAL '6 days'
    GROUP BY 1
),
